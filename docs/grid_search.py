...
def run(arg):
    ...
    with open(f"reports\\GridSearch.md", "a") as file:
        file.write(f"{dataset} {classifier_type} Settings: Scaling: {str(scaling)} Labeling/one-hot-encoding: {'labeling'if labeling else 'one-hot'} Impute missing with mean/own value: {'impute'if imputing else 'own value'}\n\n{res[res['rank_test_score']<4].to_markdown()}\n\n")
